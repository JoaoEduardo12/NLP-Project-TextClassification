...
class TextProcessing:
    ...
    def _get_nlp_docs(self):
		...
		'''
        print("\nProcessing train and text datasets with spacy.\nIt will take a while..")
    ...
    def _lemmatization(self):
        ...
        for document in range(len(self.X_train_doc) - 1):
            self.X_train_doc[document] = [token.lemma_ for token in self.X_train_doc[document] if not token.is_punct]
        for document in range(len(self.X_test_doc) - 1):
            self.X_test_doc[document] = [token.lemma_ for token in self.X_test_doc[document] if not token.is_punct]

    def _add_stop_word_vocab(self, word):
        '''
    ...
    def _remove_stop_words(self):
		...
		'''
        for document in range(len(self.X_train_doc) - 1):
            self.X_train_doc[document] = [token for token in self.X_train_doc[document] if not token in self.nlp.Defaults.stop_words]
        for document in range(len(self.X_test_doc) - 1):
            self.X_test_doc[document] = [token for token in self.X_test_doc[document] if not token in self.nlp.Defaults.stop_words]

    def _return_processed_data(self):
        '''
